...
def enrich_from_aircraft_db(icao: str, aircraft_db: AircraftDB | None) -> dict[str, str | None]:
    if not aircraft_db:
        return {}
...
def format_alert(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    alert_id = doc.get("alert_id") or f"{doc.get('flight_id', '')}:{doc.get('zone', '')}:{doc.get('rule', '')}"
    return {
        "alert_id": alert_id,
...
def alert_stats_by_flight(
    ...
    *,
    ...
    for doc in db.get_collection("alerts").find({"flight_id": {"$in": flight_ids}}):
        flight_id = doc.get("flight_id")
        alert_id = doc.get("alert_id") or f"{flight_id}:{doc.get('zone', '')}:{doc.get('rule', '')}"
...
def enrich_flight_summary(summary: dict[str, Any], aircraft_db: AircraftDB | None) -> dict[str, Any]:
...
def flight_summary(doc: dict[str, Any], last_tel: dict[str, Any] | None,
    ...
    alert_stats = live_alert_stats(active_alerts) if is_live and active_alerts else doc.get("alert_stats")
    summary = {
        "flight_id": doc["_id"],
        "icao": icao,
...
def enrich_flight_detail(flight_data: dict[str, Any], icao: str,
                         aircraft_db: AircraftDB | None) -> dict[str, Any]:
    enriched = enrich_from_aircraft_db(icao, aircraft_db)
    ...
    result = {
        **flight_data,
        ...
        "aircraft_type": flight_data.get("aircraft_type") or enriched.get("aircraft_type"),
