...
def _cmd_status(client: pymongo.MongoClient | None, live_store: Any = None) -> None:
    ...
    if live_store is not None:
        try:
            live_flights = live_store.get_flights() if hasattr(live_store, "get_flights") else live_store.get_live_flights()
...
def format_dump1090_table(live_flights: list[dict[str, Any]], aircraft_db: AircraftDB | None = None) -> str:
    ...
    for flight in live_flights:
        ...
        spd = flight.get("speed") or flight.get("horizontal_speed")
        spd_str = f"{int(spd)}" if spd is not None else "N/A"

        hdg = flight.get("heading") or flight.get("track")
...
# Backwards compatibility aliases for module imports
run_dump1090_loop = run_live_loop
run_dump1090_cmd = run_live_cmd
