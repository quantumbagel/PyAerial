...
while True:
    ...
    if verb == "about":
        ...
        if len(prompt_breakdown) == 1:
            print("No supplement supplied!")
            continue
        first_argument = prompt_breakdown[1]
        if first_argument == "planes":
            size = client.admin.command('listDatabases')
            print(
                f"Planes ({len([i for i in size['databases'] if i['name'] not in EXCLUDED_DATABASES])}): {' '.join([d['name'] for d in size['databases'] if d['name'] not in EXCLUDED_DATABASES])}")
        elif first_argument == "plane":
            plane_id = prompt_breakdown[2]
            planes = client.list_database_names()
            if plane_id not in planes:
                print(f"I don't know the plane {plane_id}! Usage: list plane <plane id>")
                continue
            ...
            print(f"I don't know the supplement \"{first_argument}\"!")
    elif verb == "reset":
        if len(prompt_breakdown) == 1:
            if not last_reset or reset_for != "":
                print("Are you sure you want to reset the database? Run \"reset\" again to reset.")
                last_reset = True
                reset_for = ""
                ...
                print(f"Database successfully reset. Dropped {len(names)} planes.")
                last_reset = False
        else:
            if not last_reset or reset_for != prompt_breakdown[1]:
                print(f"Are you sure you want to delete the plane {prompt_breakdown[1]}? Run \"reset\" again to reset.")
                last_reset = True
                reset_for = prompt_breakdown[1]
            else:
                names = list([i for i in client.list_database_names() if i not in EXCLUDED_DATABASES])
                client.drop_database(prompt_breakdown[1])
                print(f"Database successfully reset. Dropped plane {prompt_breakdown[1]}.")
                last_reset = False
                reset_for = ""
    elif verb == "exit":
        ...
        if first_argument == "plane":
            ...
            db = client.get_database(plane_id)
            flight = db.get_collection(flight_id)
            flights = db.list_collection_names()
            if flight_id not in flights:
                print(f"I don't know the flight id \"{flight_id}\".\nAll flights for this plane:\n{' '.join(flights)}")
                continue
            flight_contents = list(flight.find({}, {"_id": 0}))
            for flight_content in flight_contents:
                category = flight_content[STORAGE_CATEGORY]
            ...
            if plane not in planes:
                print(f"I don't know the plane {plane}! Usage: dump opensky <plane id>")
                continue
            print(get_airplane_info(plane))
            pass
        else:
            print(f"I don't know the keyword {first_argument}")
    if verb != "reset":
        last_reset = False
